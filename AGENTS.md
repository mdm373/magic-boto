...
`ANTHROPIC_API_KEY` is only required for tag **sweep** and **audit** flows (`sweep.*`, `audit.*`, and related MCP tools). All other tasks only need the Postgres vars.
...
```powershell
uv run invoke fetch                    # sync fetch & ingest MTGJSON catalog (see also MCP async jobs in PROJECT-PLAN)
...
For a fuller command list, see the root **[README.md](README.md)** (Quick start + Common workflows).

## Quick orientation

Two processes, one image (`tools_api/`): **`app.cmd.serve_http:app`** (FastAPI, default port 8000) and **`app.cmd.serve_mcp.asgi:app`** (MCP streamable HTTP, default port 8765) — see root **`docker-compose.yml`**. Shared domain code is in **`app/services`**, **`app/api_schema`**, **`app/models`**, **`app/db`**.

MCP tools live under **`app/mcp_tooling/`**: one `register_*_tools(app_mcp)` per module, aggregated from **`app/mcp_tooling/tools.py`** into **`app/mcp_tooling/server.py`** (FastMCP factory).
...
- **Use package barrels for public APIs.** For packages like `api_schema/`, re-export commonly used classes in the package `__init__.py` using `__all__` so consumers can import from the package root.
...
- **No vague “helpers” dumping grounds.** Do not add generic top-level modules (e.g. `helpers.py`, `utils.py`, `openapi_helpers.py`). Put small utilities in the package that owns their use (e.g. OpenAPI description text beside the relevant `api_schema` module).

## Python (tools_api/)

- **Sibling imports.** Within the same package (e.g. modules under `app/models/`), prefer relative imports (`from .base import Base`, `from .sibling import Foo`) instead of fully qualified `app....` paths. Use absolute `app....` imports when crossing package boundaries (e.g. `app/services` → `app/api_schema`). Do not import through the `app.models` package barrel from inside `app.models` (e.g. avoid `from app.models import FooModel` in a sibling module); that re-enters `__init__` and causes cycles. Import the defining module instead (`from .foo_model import FooModel`).
...
- **Migrations.** Create or edit files under `tools_api/migrations/versions/`; apply with `uv run invoke migrate` from `tools_api/`. See **`tools_api/migrations/README.md`** for the short workflow. Create or edit migration files as requested; **do not run migrations unless the user confirms.**

- **Catalog data** lives in the **`magic_boto`** schema. After migrations, load MTGJSON into it via **`uv run invoke fetch`** (sync CLI) or the async job pipeline (see **`docs/PROJECT-PLAN.md`**).
...
- **tools_api HTTP vs MCP.** The FastAPI app is loaded via `uvicorn app.cmd.serve_http:app` (see `docker-compose.yml` **`tools_api`**). Streamable MCP HTTP: `uvicorn app.cmd.serve_mcp.asgi:app` (see **`tools_mcp`**). Shared wiring for FastMCP lives in **`app/mcp_tooling/server.py`**. **Stdio** MCP: `uv run python -m app.cmd.serve_mcp.stdio`. Shared domain code stays at `tools_api/app/` (`services`, `api_schema`, `db`, `models`).
...
- **LLM / orchestration.** There is **no in-repo agent or chat UI**. Use an external MCP-capable client (Cursor, Claude Desktop, etc.) pointed at **`http://<host>:<port>/mcp`** (see **`TOOLS_MCP_PORT`**). Put **system prompts and task instructions in Markdown** under **`tasks/`** (or `docs/`) and reference them from your workflow or client rules (e.g. **`tasks/deck-building-instructions.md`**). For **deck building and inventory/deck edits**, follow that file: **MCP-only** for those flows—**do not** answer by running ad-hoc Python or SQL against Postgres instead of MCP tools.
