...
- **Always run Ruff when making code changes.** After any create or edit of Python under `tools_api/` or `agent_api/` (including `api/app/`, `api/migrations/`, and `api/tasks/`), run from the `api/` directory—do not skip this:
  - `uv run ruff check .`
  - `uv run ruff format .`
...
- **Env before run.** The app does not load `.env` itself. Populate the environment first (e.g. from repo root run `.\scripts\load-env.ps1` or source `.env`), then start the server or run tasks. Docker Compose can inject env via its `env_file` / `environment` config.
- **Invoke tasks.** From `api/`, `uv run invoke serve.start` and `uv run invoke migrate` run the server and migrations (after env is populated).
