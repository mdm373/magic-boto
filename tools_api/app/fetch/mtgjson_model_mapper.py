...
from .mtgjson_schema import MtgJsonSchema
...
class MappedSetPayload:
    ...
    card_types: Sequence[MagicBotoCardTypeModel]
    card_subtypes: Sequence[MagicBotoCardSubtypeModel]
    card_supertypes: Sequence[MagicBotoCardSupertypeModel]
    card_keywords: Sequence[MagicBotoCardKeywordModel]
    card_meta: Sequence[MagicBotoCardMetaModel]


class MtgJsonModelMapper:
    """Parse MTGJSON on disk and build ORM rows for ``magic_boto``."""

    def map_editions(self, path: Path) -> Sequence[MagicBotoEditionModel]:
        doc = MtgJsonSchema.SetListDocument.model_validate_json(path.read_text(encoding="utf-8"))
        return [
            MagicBotoEditionModel(set_code=item.code.strip().upper(), name=item.name)
    ...
    def map_set_payload(self, *, path: Path, set_code: str) -> MappedSetPayload:
        ...
        cards: list[MagicBotoCardModel] = []
        types: list[MagicBotoCardTypeModel] = []
        subtypes: list[MagicBotoCardSubtypeModel] = []
        supertypes: list[MagicBotoCardSupertypeModel] = []
        keywords: list[MagicBotoCardKeywordModel] = []
        meta: list[MagicBotoCardMetaModel] = []
        ...
        for i, card in enumerate(doc.data.cards):
            ...
            for t in card.types:
                ...
                try:
                    ...
                    continue
                types.append(MagicBotoCardTypeModel(card_id=card.uuid, card_type=type_value))
            for st in card.subtypes:
                token = st.strip().lower()
                if token:
                    subtypes.append(
                        MagicBotoCardSubtypeModel(card_id=card.uuid, card_subtype=token)
                    )
            ...
            for token in _keyword_tokens(card.keywords):
                keywords.append(MagicBotoCardKeywordModel(card_id=card.uuid, card_keyword=token))
