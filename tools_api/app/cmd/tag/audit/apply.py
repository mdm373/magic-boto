...
from app.repository import CardTagRepo, TagAuditRepo, TagRepo
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply audit suggestion to a tag's description."
    )
            ...
            if delete_side_tags:
                for suffix in ("_unsure", "_excluded"):
                    side = await session.scalar(
                        select(TagModel).where(TagModel.name == f"{tag_name}{suffix}")
                    )
                    if side is not None:
