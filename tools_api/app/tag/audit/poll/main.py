...
from app.services import create_batch_poller, create_tag_audit_service


class _AuditPollProvider:
    def __init__(self) -> None:
        self._service = create_tag_audit_service()
    ...
    async def fetch_batch_ids(
        ...
        audit = await self._service.get_audit(session, uuid.UUID(args.audit_id))
        if audit is None:
