...
from app.services import create_batch_service, create_tag_audit_service, create_tag_service
from app.tag.batch.client import Request, create_batch_client
...
_PROMPTS_DIR = Path(__file__).parents[1]
...
async def _run(
    ...
    async with session_factory() as session:
        audit = await _audit_service.create_audit(session, tag.id)
        batch = await _batch_service.create_batch(session, anthropic_batch_id)
