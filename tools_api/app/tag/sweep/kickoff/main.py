...
from app.tag.batch.client import BATCH_DIR, BatchApiClient, Request, create_batch_client
from app.tag.card_payload import cards_to_csv
from settings import get_settings

_SYSTEM_PROMPT_TEMPLATE = (BATCH_DIR / "system_prompt.md").read_text().strip()
_OUTPUT_SCHEMA_PATH = BATCH_DIR / "output_schema.json"
...
async def _submit_chunks(
    ...
    async with session_factory() as session:
        cards_by_oracle_id = {
            c.oracle_id: c
            for c in await _card_service.fetch_cards_by_oracle_ids(session, all_oracle_ids)
        }
...
async def _run(tag_name: str, limit: int, reenqueue_failed: bool) -> uuid.UUID | None:
    ...
    async with session_factory() as session:
        ...
        if run is None:
            run = await _sweep_run_service.create_sweep(session, tag.id)
            await session.commit()
    ...
    if reenqueue_failed:
        async with session_factory() as session:
            failed_ids = list(
                await _sweep_run_service.get_failed_oracle_ids_for_sweep(session, run_id)
            )
            if not failed_ids:
